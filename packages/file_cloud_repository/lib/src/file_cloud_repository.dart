...
class FileCloudRepository<CA extends CloudApi> {
  ...
  final CA cloudApi;
  final _filesStreamController = BehaviorSubject<List<File>>.seeded(const []);
  ...
  Future<void> refresh() async {
    try {
      ...
      _filesStreamController.add(localFiles.map((e) => e.file).toList());
      ...
      cloudApi.listFile().then((files) {
        for (var nativeFile in files) {
          if (nativeFile is drive.File) {
            ...
            if (localFile != null) {
              if (localFile.version != nativeFile.version) {
                needSyncFiles[FileSyncOper.save]!.add(localFile);
              }
            } else {
              localFiles.add(FileCloud(file: File(nativeFile.name ?? 'tmp'), version: nativeFile.version,));
            }
          }
        }
        _filesStreamController.add(localFiles.map((e) => e.file).toList());
  ...
  Stream<List<File>> getFiles() => _filesStreamController.asBroadcastStream();
  ...
  Future deleteFile(File file) async {
    ...
    // return cloudApi.deleteFile(basename(file.path));
  }
}
