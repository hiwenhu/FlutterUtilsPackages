...
enum _TimePickerMode { hour, minute, second }
...
class _TimePickerFragmentContext {
  ...
  final TimeOfDayWithSec selectedTime;
  final _TimePickerMode mode;
  final ValueChanged<TimeOfDayWithSec> onTimeChange;
...
class _TimePickerHeader extends StatelessWidget {
  ...
  final ValueChanged<TimeOfDayWithSec> onChanged;
  final GestureTapCallback onHourDoubleTapped;
  final GestureTapCallback onMinuteDoubleTapped;
  final GestureTapCallback onSecondDoubleTapped;
  final bool use24HourDials;
  ...
  Widget build(BuildContext context) {
    ...
    final _TimePickerFragmentContext fragmentContext =
      ...
      onSecondDoubleTapped: onSecondDoubleTapped,
              ...
              SizedBox(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    ...
                    Expanded(
                        child:
                            _SecondControl(fragmentContext: fragmentContext)),
                  ],
                ),
              ),
              if (!use24HourDials &&
...
class _HourMinuteSecondControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle style =
        timePickerTheme.hourMinuteTextStyle ?? themeData.textTheme.headline4!;
...
class _HourControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String formattedHour = localizations.formatHour(
      fragmentContext.selectedTime.toTimeOfDay(),
      alwaysUse24HourFormat: alwaysUse24HourFormat,
    );

    TimeOfDayWithSec hoursFromSelected(int hoursToAdd) {
      if (fragmentContext.use24HourDials) {
        ...
        return fragmentContext.selectedTime.replacing(
          hour: (selectedHour + hoursToAdd) % TimeOfDayWithSec.hoursPerDay,
        );
      } else {
        ...
        return fragmentContext.selectedTime.replacing(
          hour: periodOffset +
              (hours + hoursToAdd) % TimeOfDayWithSec.hoursPerPeriod,
        );
      }
    }

    final TimeOfDayWithSec nextHour = hoursFromSelected(1);
    final String formattedNextHour = localizations.formatHour(
      nextHour.toTimeOfDay(),
      alwaysUse24HourFormat: alwaysUse24HourFormat,
    );
    final TimeOfDayWithSec previousHour = hoursFromSelected(-1);
    final String formattedPreviousHour = localizations.formatHour(
      previousHour.toTimeOfDay(),
      alwaysUse24HourFormat: alwaysUse24HourFormat,
    );
    ...
    return Semantics(
      ...
      child: _HourMinuteSecondControl(
...
class _StringFragment extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle hourMinuteStyle =
        timePickerTheme.hourMinuteTextStyle ?? theme.textTheme.headline4!;
...
class _MinuteControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String formattedMinute =
        localizations.formatMinute(fragmentContext.selectedTime.toTimeOfDay());
    final TimeOfDayWithSec nextMinute = fragmentContext.selectedTime.replacing(
      minute: (fragmentContext.selectedTime.minute + 1) %
          TimeOfDayWithSec.minutesPerHour,
    );
    final String formattedNextMinute =
        localizations.formatMinute(nextMinute.toTimeOfDay());
    final TimeOfDayWithSec previousMinute =
        fragmentContext.selectedTime.replacing(
      minute: (fragmentContext.selectedTime.minute - 1) %
          TimeOfDayWithSec.minutesPerHour,
    );
    final String formattedPreviousMinute =
        localizations.formatMinute(previousMinute.toTimeOfDay());
...
///
/// When tapped changes time picker dial mode to [_TimePickerMode.second].
class _SecondControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String formattedNextSecond = localizations.formatMinute(
        TimeOfDay(hour: nextSecond.hour, minute: nextSecond.second));
    final TimeOfDayWithSec previousSecond =
        fragmentContext.selectedTime.replacing(
      second: (fragmentContext.selectedTime.second - 1) %
          TimeOfDayWithSec.secondsPerMinute,
    );
    final String formattedPreviousSecond = localizations.formatMinute(
        TimeOfDay(hour: previousSecond.hour, minute: previousSecond.second));

    return Semantics(
      excludeSemantics: true,
      value:
          '${localizations.timePickerMinuteModeAnnouncement} $formattedSecond',
      ...
      child: _HourMinuteSecondControl(
        isSelected: fragmentContext.mode == _TimePickerMode.second,
        text: formattedSecond,
        onTap: Feedback.wrapForTap(
            () => fragmentContext.onModeChange(_TimePickerMode.second),
            context)!,
        onDoubleTap: fragmentContext.onMinuteDoubleTapped,
      ),
    );
...
class _DayPeriodControl extends StatelessWidget {
  ...
  void _togglePeriod() {
    final int newHour = (selectedTime.hour + TimeOfDayWithSec.hoursPerPeriod) %
        TimeOfDayWithSec.hoursPerDay;
    final TimeOfDayWithSec newTime = selectedTime.replacing(hour: newHour);
    onChanged(newTime);
  }
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  double _getThetaForTime(TimeOfDayWithSec time) {
    final int hoursFactor = widget.use24HourDials
        ? TimeOfDayWithSec.hoursPerDay
        : TimeOfDayWithSec.hoursPerPeriod;
    double fraction = 0;
    switch (widget.mode) {
      ...
      case _TimePickerMode.minute:
        fraction = (time.minute / TimeOfDayWithSec.minutesPerHour) %
            TimeOfDayWithSec.minutesPerHour;
        break;
      case _TimePickerMode.second:
        fraction = (time.second / TimeOfDayWithSec.secondsPerMinute) %
            TimeOfDayWithSec.secondsPerMinute;
        break;
      default:
    }
  ...
  TimeOfDayWithSec _getTimeForTheta(double theta,
      {bool roundMinutes = false, required bool roundSeconds}) {
    final double fraction = (0.25 - (theta % _kTwoPi) / _kTwoPi) % 1.0;
    if (widget.mode == _TimePickerMode.hour) {
      int newHour;
      if (widget.use24HourDials) {
        newHour = (fraction * TimeOfDayWithSec.hoursPerDay).round() %
            TimeOfDayWithSec.hoursPerDay;
      } else {
        newHour = (fraction * TimeOfDayWithSec.hoursPerPeriod).round() %
            TimeOfDayWithSec.hoursPerPeriod;
      ...
      int minute = (fraction * TimeOfDayWithSec.minutesPerHour).round() %
          TimeOfDayWithSec.minutesPerHour;
      if (roundMinutes) {
        // Round the minutes to nearest 5 minute interval.
        minute = ((minute + 2) ~/ 5) * 5 % TimeOfDayWithSec.minutesPerHour;
      }
      return widget.selectedTime.replacing(minute: minute);
    } else {
      int second = (fraction * TimeOfDayWithSec.secondsPerMinute).round() %
          TimeOfDayWithSec.secondsPerMinute;
      if (roundSeconds) {
        // Round the minutes to nearest 5 minute interval.
        second = ((second + 2) ~/ 5) * 5 % TimeOfDayWithSec.secondsPerMinute;
      }
      return widget.selectedTime.replacing(second: second);
    }
  }

  TimeOfDayWithSec _notifyOnChangedIfNeeded(
      {bool roundMinutes = false, bool roundSeconds = false}) {
    final TimeOfDayWithSec current = _getTimeForTheta(_theta.value,
        roundMinutes: roundMinutes, roundSeconds: roundSeconds);
  ...
  void _handleTapUp(TapUpDetails details) {
    ...
    final TimeOfDayWithSec newTime =
    ...
    _animateTo(_getThetaForTime(_getTimeForTheta(_theta.value,
        roundMinutes: true, roundSeconds: true)));
    _dragging = false;
    _position = null;
    _center = null;
  }
  ...
  void _selectHour(int hour) {
    ...
    if (widget.mode == _TimePickerMode.hour && widget.use24HourDials) {
      ...
      if (widget.selectedTime.period == DayPeriod.am) {
        ...
        time = TimeOfDayWithSec(
            hour: hour + TimeOfDayWithSec.hoursPerPeriod,
            minute: widget.selectedTime.minute,
            second: widget.selectedTime.second);
      }
    }
  ...
  void _selectMinute(int minute) {
    ...
    final TimeOfDayWithSec time = TimeOfDayWithSec(
  ...
  static const List<TimeOfDayWithSec> _amHours = <TimeOfDayWithSec>[
    TimeOfDayWithSec(hour: 12, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 1, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 2, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 3, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 4, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 5, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 6, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 7, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 8, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 9, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 10, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 11, minute: 0, second: 0),
  ];

  static const List<TimeOfDayWithSec> _twentyFourHours = <TimeOfDayWithSec>[
    TimeOfDayWithSec(hour: 0, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 2, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 4, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 6, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 8, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 10, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 12, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 14, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 16, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 18, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 20, minute: 0, second: 0),
    TimeOfDayWithSec(hour: 22, minute: 0, second: 0),
  ];
  ...
  List<_TappableLabel> _build24HourRing(TextTheme textTheme, Color color) =>
      <_TappableLabel>[
        for (final TimeOfDayWithSec timeOfDay in _twentyFourHours)
          _buildTappableLabel(
            textTheme,
            color,
            timeOfDay.hour,
            localizations.formatHour(timeOfDay.toTimeOfDay(),
  ...
  List<_TappableLabel> _build12HourRing(TextTheme textTheme, Color color) =>
      <_TappableLabel>[
        for (final TimeOfDayWithSec timeOfDay in _amHours)
          _buildTappableLabel(
            textTheme,
            color,
            timeOfDay.hour,
  ...
  List<_TappableLabel> _buildMinutes(TextTheme textTheme, Color color) {
    const List<TimeOfDayWithSec> minuteMarkerValues = <TimeOfDayWithSec>[
      TimeOfDayWithSec(hour: 0, minute: 0, second: 0),
      TimeOfDayWithSec(hour: 0, minute: 5, second: 0),
      TimeOfDayWithSec(hour: 0, minute: 10, second: 0),
      TimeOfDayWithSec(hour: 0, minute: 15, second: 0),
      TimeOfDayWithSec(hour: 0, minute: 20, second: 0),
      TimeOfDayWithSec(hour: 0, minute: 25, second: 0),
      TimeOfDayWithSec(hour: 0, minute: 30, second: 0),
      TimeOfDayWithSec(hour: 0, minute: 35, second: 0),
      TimeOfDayWithSec(hour: 0, minute: 40, second: 0),
      TimeOfDayWithSec(hour: 0, minute: 45, second: 0),
      TimeOfDayWithSec(hour: 0, minute: 50, second: 0),
      TimeOfDayWithSec(hour: 0, minute: 55, second: 0),
    ];

    return <_TappableLabel>[
      for (final timeOfDay in minuteMarkerValues)
        _buildTappableLabel(
          textTheme,
          color,
          timeOfDay.minute,
          localizations.formatMinute(timeOfDay.toTimeOfDay()),
          () {
  ...
  List<_TappableLabel> _buildSeonds(TextTheme textTheme, Color color) {
    const List<TimeOfDayWithSec> minuteMarkerValues = <TimeOfDayWithSec>[
      TimeOfDayWithSec(hour: 0, minute: 0, second: 0),
      TimeOfDayWithSec(hour: 0, minute: 0, second: 5),
      TimeOfDayWithSec(hour: 0, minute: 0, second: 10),
      TimeOfDayWithSec(hour: 0, minute: 0, second: 15),
      TimeOfDayWithSec(hour: 0, minute: 0, second: 20),
      TimeOfDayWithSec(hour: 0, minute: 0, second: 25),
      TimeOfDayWithSec(hour: 0, minute: 0, second: 30),
      TimeOfDayWithSec(hour: 0, minute: 0, second: 35),
      TimeOfDayWithSec(hour: 0, minute: 0, second: 40),
      TimeOfDayWithSec(hour: 0, minute: 0, second: 45),
      TimeOfDayWithSec(hour: 0, minute: 0, second: 50),
      TimeOfDayWithSec(hour: 0, minute: 0, second: 55),
    ];

    return <_TappableLabel>[
      for (final TimeOfDayWithSec timeOfDay in minuteMarkerValues)
        _buildTappableLabel(
          textTheme,
          color,
          timeOfDay.second,
          localizations.formatMinute(
              TimeOfDay(hour: timeOfDay.hour, minute: timeOfDay.second)),
          () {
            _selectSecond(timeOfDay.second);
          },
        ),
    ];
  }
  ...
  Widget build(BuildContext context) {
    ...
    switch (widget.mode) {
      ...
      case _TimePickerMode.second:
        ...
        primaryLabels = _buildSeonds(theme.textTheme, primaryLabelColor);
        secondaryLabels = _buildSeonds(theme.textTheme, secondaryLabelColor);
        break;
    }
...
class _TimePickerInput extends StatefulWidget {
  ...
  final TimeOfDayWithSec initialSelectedTime;
...
class _TimePickerInputState extends State<_TimePickerInput>
  ...
  late final RestorableTimeOfDayWithSec _selectedTime =
      RestorableTimeOfDayWithSec(widget.initialSelectedTime);
  ...
  void _handleHourSavedSubmitted(String? value) {
    ...
    if (newHour != null) {
      _selectedTime.value = TimeOfDayWithSec(
          hour: newHour,
          minute: _selectedTime.value.minute,
          second: _selectedTime.value.second);
  ...
  void _handleMinuteChanged(String value) {
    ...
    if (newMinute != null && value.length == 2) {
  ...
  void _handleSecondSavedSubmitted(String? value) {
    final int? newMinute = _parseSecond(value);
    if (newMinute != null) {
      _selectedTime.value = TimeOfDayWithSec(
          hour: _selectedTime.value.hour,
          minute: _selectedTime.value.minute,
  ...
  void _handleDayPeriodChanged(TimeOfDayWithSec value) {
  ...
  String? _validateSecond(String? value) {
    ...
    setState(() {
      secondHasError.value = newSecond == null;
    });
    // This is used as the validator for the [TextFormField].
    // Returning an empty string allows the field to go into an error state.
    // Returning null means no error in the validation of the entered text.
...
class _HourTextField extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return _HourMinuteSecondTextField(
      restorationId: restorationId,
      selectedTime: selectedTime,
      fieldMode: _TextFieldMode.hour,
      autofocus: autofocus,
      style: style,
...
class _MinuteTextField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _HourMinuteSecondTextField(
      restorationId: restorationId,
      selectedTime: selectedTime,
      fieldMode: _TextFieldMode.minute,
      autofocus: autofocus,
      style: style,
...
class _SecondTextField extends StatelessWidget {
  const _SecondTextField({
    Key? key,
    required this.selectedTime,
    required this.style,
    required this.autofocus,
  ...
  Widget build(BuildContext context) {
    return _HourMinuteSecondTextField(
      restorationId: restorationId,
      selectedTime: selectedTime,
      fieldMode: _TextFieldMode.second,
      autofocus: autofocus,
      style: style,
      semanticHintText: secondLabelText ??
          MaterialLocalizations.of(context).timePickerMinuteLabel,
      validator: validator,
      onSavedSubmitted: onSavedSubmitted,
    );
...
enum _TextFieldMode { hour, minute, second }

class _HourMinuteSecondTextField extends StatefulWidget {
  const _HourMinuteSecondTextField({
    Key? key,
    required this.selectedTime,
  ...
  final TimeOfDayWithSec selectedTime;
  final _TextFieldMode fieldMode;
  final bool? autofocus;
  final TextStyle style;
  ...
  @override
  _HourMinuteSecondTextFieldState createState() =>
      _HourMinuteSecondTextFieldState();
}

class _HourMinuteSecondTextFieldState extends State<_HourMinuteSecondTextField>
    with RestorationMixin {
  ...
  String get _formattedValue {
    ...
    switch (widget.fieldMode) {
      case _TextFieldMode.hour:
        return localizations.formatHour(
          widget.selectedTime.toTimeOfDay(),
          alwaysUse24HourFormat: alwaysUse24HourFormat,
        );
      case _TextFieldMode.minute:
        return localizations.formatMinute(
          widget.selectedTime.toTimeOfDay(),
        );
      case _TextFieldMode.second:
        return localizations.formatMinute(
          TimeOfDay(
              hour: widget.selectedTime.hour,
              minute: widget.selectedTime.second),
        );
    }
  }
...
/// [Navigator.pop].
class TimePickerWithSecDialog extends StatefulWidget {
  /// Creates a material time picker.
  ///
  /// [initialTime] must not be null.
  const TimePickerWithSecDialog({
    Key? key,
  ...
  /// The time initially selected when the dialog is shown.
  final TimeOfDayWithSec initialTime;
  ...
  /// Restoration ID to save and restore the state of the [TimePickerWithSecDialog].
  ///
  ...
  @override
  State<TimePickerWithSecDialog> createState() =>
      _TimePickerWithSecDialogState();
}
...
class _TimePickerWithSecDialogState extends State<TimePickerWithSecDialog>
    with RestorationMixin {
  ...
  RestorableTimeOfDayWithSec get selectedTime => _selectedTime;
  late final RestorableTimeOfDayWithSec _selectedTime =
      RestorableTimeOfDayWithSec(widget.initialTime);
  ...
  void _handleTimeChanged(TimeOfDayWithSec value) {
    _vibrate();
    setState(() {
      _selectedTime.value = value;
    });
  ...
  void _handleMinuteSelected() {
    setState(() {
      _mode.value = _TimePickerMode.second;
    });
  }
...
///
/// ```dart
/// Future<TimeOfDayWithSec?> selectedTime = showTimeWithSecPicker(
...
///
/// ```dart
/// Future<TimeOfDay?> selectedTimeRTL = showTimeWithSecPicker(
///   context: context,
///   initialTime: TimeOfDayWithSec.now(),
...
///
/// ```dart
/// Future<TimeOfDay?> selectedTime24Hour = showTimeWithSecPicker(
///   context: context,
///   initialTime: const TimeOfDayWithSec(hour: 10, minute: 47, second: 55),
///   builder: (BuildContext context, Widget? child) {
///     return MediaQuery(
...
/// See also:
///
///  * [showTimeWithSecPicker], which shows a dialog that contains a material design
///    date picker.
...
Future<TimeOfDayWithSec?> showTimeWithSecPicker({
  required BuildContext context,
  required TimeOfDayWithSec initialTime,
  ...
  final Widget dialog = TimePickerWithSecDialog(
  ...
  return showDialog<TimeOfDayWithSec>(
    context: context,
